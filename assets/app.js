...
(function () {
  ...
  function viewRace(id) {
    ...
    return `<div class="breadcrumb"><a href="#/races">${T('Races', 'Contiendas')}</a> › ${esc(r.title)}</div>
      ...
      ${(r.events || []).length ? `<div class="card"><h3 style="margin:0 0 6px">${T('Debates and forums', 'Debates y foros')}</h3><ul style="margin:0">${r.events.map(e => `<li><strong>${esc(e.date || '')}</strong>${e.host ? ` · ${esc(e.host)}` : ''}${e.who ? ` · ${esc(e.who)}` : ''}${sourcesHtml(e.sources)}</li>`).join('')}</ul></div>` : ''}
